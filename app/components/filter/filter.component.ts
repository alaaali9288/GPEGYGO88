...
export class filter {
    ...
    constructor(private route: ActivatedRoute, private _TripTypeService: TripTypeService, private _PlaceService: PlaceService) { }
    ...
    chooseTag(event: any) {
        if (this.keyword === "" && event.target.checked) {
            ...
            this.places = this.places.filter((item: any) => item.holidayType.findIndex((tag: any) => tag._id === tagId) !== -1
                && item.price <= this.priceFilter
                && (item.placeName.toLowerCase().includes(this.keyword.toLowerCase()) ||
